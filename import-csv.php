    ...
    if (isset($_FILES["file"])) {
        ...
        if ($num_cols == $num_cols_in_union_csv && $num_rows > 1) {
            ...
            for ($i = 1; $i < $num_rows; $i++) {
                ...
                $query = "INSERT INTO `qr_attendee`(`event_id`, `year`, `date`, `cid`, `login`, `first_name`, `surname`, `email`, `product_name`, `price`, `quantity_purchased`, `quantity_collected`) VALUES($event_id, '" . $csv_array[$i][0] . "'," . "'2000-01-01'" . "," . (int)$csv_array[$i][3] . ",'" . $csv_array[$i][4] . "','" . $csv_array[$i][5] . "','" . $csv_array[$i][6] . "','" . $csv_array[$i][7] . "','" . $csv_array[$i][8] . "'," . (int)$csv_array[$i][9] . "," . (int)$csv_array[$i][10] . "," . 0 . ")";                //STR_TO_DATE(?, '%m/%d/%Y')
